...
HAVE_XDELTA=auto           # .xdelta (VCDIFF) softpatching support (requires softpatching)
...
HAVE_ROMX=auto             # ROMX 0.2.0 frontend support via libromx
...
HAVE_RAAC=yes        	   # raac AAC audio support
...
HAVE_RZSTD=yes             # Use the built-in Zstandard codec instead of the reference library
...
C89_SPIRV_CROSS=no
HAVE_BUILTINSPIRV_CROSS=auto # Bake in SPIRV-Cross support
C89_BUILTINSPIRV_CROSS=no
HAVE_METAL=no              # Metal support (macOS-only)
C89_METAL=no
HAVE_NETWORK_VIDEO=no
HAVE_HUB75=no              # Raspberry Pi HUB75 RGB LED matrix video driver
C89_HUB75=no
