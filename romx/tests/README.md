...
- flat single-file SAVE slots (including nested paths that must not be
  auto-grouped) and PSP directory slots, including slot counts, complete
  member lists, stable IDs, export paths, rescans, import, replace, delete
  and capacity-failure rollback;
...
`test_psp_save_slots`,
`test_ordinary_and_archive_regressions` and `test_repeated_lifecycle` (the
current suite reports 124 checks).
